...
def analyze_apps(root_path, author_name, author_email, countries):
    ...
    for e in countries:
        country = e['code']
        country_name = e['name']
        data_path = os.path.join(root_path, 'data_{}'.format(country))
        if os.path.exists(data_path) and os.path.exists(data_path + '/apps.json'):
            covid_apps_observer.run_analysis(data_path, author_name, author_email,
                                             'COVID-related Android apps in {}'.format(country_name))


def main():
    # Arguments definition and management
    parser = argparse.ArgumentParser()
    parser.add_argument('-i', '--input', help='Path to the folder containing the data to be updated and '
                                              'the countries.json file', required=True, type=str)
